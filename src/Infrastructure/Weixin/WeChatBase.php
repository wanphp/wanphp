...
<Nonce><![CDATA[%s]]></Nonce>
  ...
  {
    //数据库取缓存
    $cachedata = $this->redis->get('weixin_jsonToken');
    if ($cachedata) $access = json_decode($cachedata);
    if (isset($access) && $access->expires_in > time()) {
    ...
    if (isset($result['access_token'])) {
      $expire = intval($result['expires_in']) - 100;//过期提前100秒，重新获取
      $token = json_encode(array('access_token' => $result['access_token'], 'expires_in' => $expire + time()));
      $this->redis->setex('weixin_jsonToken', $expire, $token);
...
//    $referer = $scheme . '://' . $host . '/';
  ...
  {
    $cachedata = $this->redis->get('weixin_jsonTicket');
    if ($cachedata) $access = json_decode($cachedata);

    if (isset($access) && $access->expires_in > time()) {
      $this->jsapi_ticket = $access->jsapi_ticket;
      return $access->jsapi_ticket;
    }

    $result = $this->httpGet('https://api.weixin.qq.com/cgi-bin/ticket/getticket?type=jsapi&{ACCESS_TOKEN}');
    if ($result) {
      $expire = isset($result['expires_in']) ? intval($result['expires_in']) - 100 : 3600;
      $ticket = json_encode(array('jsapi_ticket' => $result['ticket'], 'expires_in' => $expire + time()));
      $this->redis->setex('weixin_jsonTicket', $expire, $ticket);
